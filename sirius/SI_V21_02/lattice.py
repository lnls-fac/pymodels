...
def set_num_integ_steps(the_ring):
    ...
    for i in range(len(the_ring)):
        if the_ring[i].angle:
            ...
            the_ring[i].nr_steps = nr_steps
        elif the_ring[i].polynom_b[1] or the_ring[i].fam_name in ['fc', 'fcq', 'dipk','nlk']:
...
def set_vacuum_chamber(the_ring, mode=default_optics_mode):
    ...
    bcs = _pyaccel.lattice.find_indices(the_ring, 'fam_name', 'bc_hf')
    for i in bcs:
        e = the_ring[i]
...
def get_optics_mode(mode=default_optics_mode, version=_default_version):
