...
def create_lattice():
    ...
    # -- drifts --
    LIA  = drift('lia2', 1.5679)
    LIB  = drift('lib2', 1.3279)
    ...
    MIDA   = marker('id_enda') # marker for the extremities of IDs in long straight sections
    MIDB   = marker('id_endb') # marker for the extremities of IDs in short straight sections
    SEPTIN = marker('sept_in') # end of injection septum
    ...
    INJ = [L500,RBPM,LIA,L500,L200,SEPTIN,L800,END,START,MIA,LKK,KICKIN,LPMU, PMM, LPMD];               # high beta INJ straight section
...
def set_vacuum_chamber(the_ring):
    ...
    sept_in  = _pyaccel.lattice.find_indices(the_ring, 'fam_name', 'sept_in')[-1] # physics end of thin septum
